...
**完成紀錄（B3b，2026-09-04，分支 `b/b3b-diff-engine`；設計核准
B3B-DESIGN-REPORT，Q1–Q6 作答與裁決點 D1–D8 全項通過）**：

1. **分階與措辭**：B3b 把 B3a 的見證集 no-go 升到**全語言**——純 tools、零 Lean。
   B3a 紀錄第 1 點列給 B3b 的四件事中，本紀錄落地「`D_A = V∘U − V` 差分自動機」與
   「引擎驅動的全語言重推」兩件（單模式）；**B3c**（待排程，落 `ProjectB/`）= Lean 鏡射：
   `rdDFA`（7 態）與接受引理、`DiffAuto θ` 載體與橋定理
   `cost (DiffAuto θ) (extInM x) = θ·ΔF_B x`、圖憑證的驗證書（P1–P5 ＋ 圖 Farkas），
   以及第 8 點的 2 見證無符號定理（`decide` + `linarith` 十行）；雙模式／仿射
   （`SelCostAutomaton` 實例化）仍待排程（第 8 點 (4) 的觀察是其前置資料）。
   **保險絲未觸發**：θ-LP 不可行、憑證以純整數驗證；零 Lean、零新依賴、`.github` 零變更。
2. **構造（D1；`tools/b3b_diff.py` §B）**：`D(θ)` = 輸入側 Core Level-2 機器 ×（輸出側
   **同一台**機器，由輸入側發射位 d = outBit **同步驅動**——NOTES Q1 的關鍵觀察驗證後採用：
   `step2` 算的 d 即 B0 `U` 每步的輸出位、K→S 邊界即第一個 d = 1——於邊界啟動）× 7 態
   ranking-domain DFA。不變量 idle ⟺ 輸入側在 K。邊權向量
   v = e_{featIdx(out,d)} − e_{featIdx(in,b)}、α = 0；輸入耗盡後輸出側尚欠的 1+p 個哨兵零
   （p = 終態 (0,S,p) 的第三分量；3x+1 的位長 = n+1+p）押進 β。手算 x = 3：逐步
   −e₇／−e₁₃+e₇／−e₁₆+e₁₂／−e₈+e₇、β = e₁₂+e₆，總帳 = F_B(5) − F_B(3)；x = 5（邊界落在第二
   哨兵步）與 x = 1（拒絕）同為電池錨。可達 65 態（(in, out) 相異對 26、輸入分量恰 S8）；
   trim 後 39 態／75 邊（some 0／some 1／none = 27／28／20）／接受態 4，輸入分量恰為 A 的終末態對。
3. **域條款（D2）**：NOTES Q2 的 (a)「接受集排除 `[1]` 的終態組合」**不成立**——x = 1 的終態
   ((0,S,1),(2,K,0)) 被 x = 3, 13, 53, 213, …（(5·2^j − 1)/3、j 奇，Todd = 5）整族共用；
   (c) 引擎層過濾不可行（Todd 1 = 1 ⟹ D_θ(1) ≡ 0，引擎對每個 θ 皆以 x = 1 fail）。採 (b′)：
   B0 `lstep` 加一態 `one`（恰讀過 `[1]`，讀哨兵入 dead），接受語言恰 {extInM x : x 奇 ∧ x > 1}
   （x < 16384 全體驗證）；Lean `rdDFA` 記 B3c。
4. **成本橋（Q3 通道分離）**：向量形 `vec_cost(extInM x) = ΔF_B(x)` 對 x < 8192 全體
   （ΔF_B 經 b3_attest 的 B 側自含實作）；引擎通道：`D(θ)` 經 b2 `mk_automaton` 實例化、
   成本經 b2_engine `_cost`，對 4 組固定種子隨機有理 θ（含負值）× 奇 x < 2048 與 θ·ΔF_B
   逐項相等。
5. **θ-LP 與圖憑證（D3／D4；交付 4）**：simple cycles **328**（長 1–18；相異權向量 175）、
   elementary 接受路徑 **8269**（長 4–29；相異向量 5140）。∃θ ≥ 0 AllNeg(θ) ⟺
   {θ ≥ 0, θ·v(C) ≤ 0, θ·w(p) ≤ −1} 可行（走行 = elementary 路徑 + simple cycles；有限多條
   嚴格式齊次縮放）。求解：**自建精確兩階段單純形**（Bland、`Fraction`、純標準庫）解枚舉形的
   Farkas 對偶（20 × 5315，0.8 s）；對偶可行 ⟹ 原 LP 不可行、y 直接落在生成元；對偶不可行
   ⟹ 其 Farkas 乘子即可行 θ（保險絲）。**LP 求解器不受信任**：兩種輸出都由呼叫端純有理／
   純整數驗證（設計階段 sympy `linprog` 曾在本實例回傳違反約束的點，故 LP 不用 sympy；
   自驗設計使其非承重）。結果**不可行**，整數憑證三種，同一驗證器（係數正整數、生成元 ∈
   枚舉集、聚合逐座標 ≥ 0、Σν > 0）：(i) LP 導出——6 循環 + 1 路徑（x = 31041）、聚合 = 0
   （只印不錨）；(ii) **極小典範對立對 (25, 315)**——ν = (1, 1)、零循環、聚合 = 0、Σν = 2
   （**入錨三件套**：ΔF_B(25) + ΔF_B(315) = 0、兩走行皆 elementary、皆在域內；相異 elementary
   路徑向量的對立對共 42 組，典範取法：每類取最小解碼 x，依 (max, min) 排序取首）；
   (iii) **B3a 提升**——W₁₀ 十條走行切環分解（5 條本身 elementary：231、323、403、551、1079；
   681 = 路徑 + 三個長 2 循環；877 + 長 3；983、1305 + 長 7；1511 + 長 6），以 λ_B 加權得
   9 路徑生成元（Σν = 34）+ 5 循環生成元（Σμ = 26）、聚合 = e₁₇（= Lean `agg_eq_e17`）——
   A-3 cycle-basis 重建在 B 語言的重現（NOTES Q4 觀察，入錨）。
6. **負向測試（交付 5）**：刪對立對一列／換一列／B3a 提升憑證一個循環係數 +1 ⟹ 驗證器紅；
   **去掉路徑列 ⟹ LP 可行**（加 Σθ_活 = 1 正規化，否則 θ = 0 平凡；單純形回 θ = e₆ 並對全部
   循環向量驗證——邊界座標 6／13 不在任何循環上，`boundary_step_unique` 的圖形式）；
   **去掉循環列 ⟹ 仍不可行**（對立對只用路徑列）——L2 單模式的障礙完全住在邊界（路徑）
   結構，B2「Karp 單獨不足」之角的 ∃θ 版本。
7. **引擎 harness（交付 3）**：25 組 θ ≥ 0（θ≡1、θ≡0、16 個活座標的 e_k、6 組固定種子隨機
   非負有理、K=1/S=2）**全數 fail**；每個見證去標記解碼回 x（`word == extInM x`）、x 奇且 > 1、
   D_θ(x) = θ·ΔF_B(x) ≥ 0 且 = 引擎成本；**最小見證 x = 3**；boundary 模式恰
   {θ≡1, θ≡0, e₆, e₁₃, K/S}、其餘 20 組 cycle 模式；最大見證 ≈ 1.56·10¹³（44 位 pump 字）。
   引擎自此對任何候選 θ（不限符號）生成全語言反例。
8. **發現與處置（D5）**：對立對 ΔF_B(25) + ΔF_B(315) = 0（Todd 25 = 19、Todd 315 = 473；
   A 側 F2∘σ 通道同樣成立）⟹ 對**任意符號** θ ∈ ℚ¹⁸ 兩步不能同時嚴格下降 ⟹ **Level 2
   單模式不存在任何符號的線性 ranking**（2 見證；A 的定理需 θ ≥ 0、10 見證）。普查
   （`--deep`，向量類口徑）：x < 2¹⁶ 相異 ΔF_B 向量 12345、對立對 269，最小三組
   (25, 315)、(441, 587)、(9, 827)（587 的走行非 simple，故 elementary 典範為 (25, 315)）；
   專案主人獨立重算證實。處置：(1) **paper 轉交**（附全事實）：main.tex 669–671 行「權重假設
   逐分量非負；自然強化為 bounded-below + gauge」——在 L2 單模式此假設可整個拿掉且只要 2 個
   見證，為 §gauge 段與 Class A 敘事的增補候選，**轉交修訂線**（REVISION-SCOPE 合規閘門），
   本 PR paper/ 零觸碰；(2) B1 節「直接紅利」在 L2 單模式被此結果涵蓋——**B1 節文字不動，
   僅此處交叉註記**；(3) Lean 定理歸 **B3c、落 ProjectB**；(4) **雙模式無符號普查（觀察層，
   不入錨）**：36 維雙模式差分（區塊 m(y) 加 F_B(y)、區塊 m(x) 減 F_B(x)，m = F_B[13]，
   即 A 的 `two_mode_delta` 口徑）x < 2¹⁶ 相異向量 16407、對立對 **30**，最小三組
   (1611, 2233)、(1787, 2537)、(2683, 3577)；38 維仿射差分（附 e_{m(y)} − e_{m(x)}）對立對
   亦 30 組、同三組（截距差對消）；(1611, 2233) 經 A 側 `two_mode_delta` 複核之和為零。
   即**雙模式與雙模式仿射模板亦有無符號 2 見證對立對**（A 的 `no_go_2mode_potential`／
   `no_go_2mode_affine_potential` 需 θ ≥ 0、12 見證）——此為觀察，Lean 化與論文處置隨
   B3c／修訂線；(5) (25, 315) 三件套入錨（第 5 點）。
9. **驗證**：`python3 tools/b3_attest.py`（含 §G）全綠 **2.0 s**（§G 1.4 s；預算 30 s）；
   `python3 tools/b3b_diff.py --deep` 本機 15 s（向量橋 x < 2¹⁶、兩種普查、θ sweep ×200 全 fail
   且全 cycle 模式、只留路徑列的對偶 LP 11 s 不可行）；`b2_engine --selftest`、其餘腳本與
   兩生成器零 diff；無 Lean 變更故 `lake build` 不適用；CI 維持 attest 一步、`.github` 零變更、
   零新依賴。
