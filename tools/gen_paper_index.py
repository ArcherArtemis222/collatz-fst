...
1. **型別快照是人工輸入，本檔絕不從 Lean 原始檔抽取敘述。**
   ...
   lake build 時檢查。
   同理，`latex` 欄（論文敘述）只影響 gen_tex——**gen_lean 絕不讀 latex 欄**，
   填寫或修改 latex 欄不得改變 PaperIndex.lean 的任何一個位元組。
...
def gen_tex(entries):
    # 在 PREAMBLE \input 本檔。它定義三樣東西：
    #   1. 每條 id 的 Lean 全名巨集 \leanname@<id>（main.tex 的 \leanref 用
    #      \csname 取用——映射由 registry 生成，不得在 main.tex 手工維護，
    #      否則重新引入 PR-0 要殺掉的漂移）；
    #   2. 每條**已填** latex 欄的敘述巨集 \leanstmt@<id>（main.tex 的
    #      \paperthm 環境取用——正文不手寫定理敘述，這是最後一條漂移通道的
    #      封口）。latex 欄內容即 LaTeX 原始碼，逐字排入、不做 tex_escape；
    #      狀態標記（STATUS_RE，如【待撰】）視為未填不排，讓 \paperthm 對
    #      未知 id 與未填 id 走同一條 TeX 硬報錯路徑；
    #   3. \TheoremIndexTable：附錄對應表（id / Lean 全名 / 檔案）。
    lines = [
    ...
    for e in entries:
        latex = str(e["latex"]).strip() if e["latex"] is not None else ""
        if latex and not STATUS_RE.match(latex):
            lines.append(rf"\csdef{{leanstmt@{e['id']}}}{{{latex}}}")
    lines += [
